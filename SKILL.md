...
- 금지: 그라데이션, 사진 풀블리드, 진한 대면적 배경, 과한 그림자/블러, 작은 저대비 텍스트, 장식이 이름 영역을 침범하는 구성, 점선/밑줄 작성 가이드
...
### 크리에이티브 원칙 — 인쇄 안전 안에서 매번 새롭게

이 스킬은 고정 템플릿 찍어내기가 아니라, 행사 무드를 인쇄 가능한 벡터 조형으로 번역하는 도구다. 다만 크리에이티브함은 작성 공백과 인쇄 안전을 침범하면 실패다.

- 매번 같은 상단 포스터 + 하단 박스 패턴으로 도망가지 않는다. 레이아웃 리듬, 상징의 위치, 여백의 형태, 코너/프레임/실루엣 조형을 행사마다 다르게 잡는다.
- 단, 그라데이션·풀블리드·사진·대면적 진한 배경은 계속 금지한다.
- 장식은 전체의 최대 1/3 안에서만 쓴다. 나머지 최소 2/3는 손글씨가 편한 빈 공간이다.
- 공백 자체를 디자인 요소로 본다. 빈 칸을 선으로 설명하지 않는다.
...
- 현장 수기용/공백형 네임택은 더 엄격하다. **전체 셀의 최소 2/3는 이름·소속을 손으로 적는 빈 공간**으로 남긴다.
- 적어도 소속명과 이름을 크게 쓸 수 있는 빈 공간이어야 한다. 유도선 2개를 넣는 방식은 기본 금지한다.
- 공백 작성란에는 점선, 밑줄, 칸 구분선, `NAME / COMPANY` 같은 필기 가이드 문구를 넣지 않는다. 그냥 빈 흰 공간이어야 한다.
...
- `1`이면 기본 라벨지(`https://link.coupang.com/a/eHTHl80SGq`) 구매 링크를 안내하고, 쿠팡 파트너스 문구를 함께 보여준 뒤 Chrome에서 바로 연다.
...
다른 기기에서 인쇄한다면: 이 파일을 카카오톡·메일 본문에 "이미지로" 보내지 마세요. 재인코딩되며 DPI가 깨져 100%에서 라벨이 잘립니다. AirDrop·파일 첨부로 보내거나, 함께 생성된 PDF를 보내 받는 기기에서 다시 PNG로 뽑으세요.
...
### DPI 메타 보존 🔒 — 전송 단계에서 깨지지 않게 (애초에 방지)

미리보기 "100%"는 픽셀이 아니라 물리 크기(`물리크기 = 픽셀수 ÷ DPI`)로 계산된다. 2479×3508px(A4 300dpi) PNG의 DPI 태그가 72로 바뀌면 미리보기는 이를 가로 약 874mm로 착각해 100% 배율에서 A4에 **왼쪽 위 귀퉁이(셀 1~2개)만** 들어오고 나머지가 잘린다. 이건 디자인·정렬·calibration 문제가 아니므로 calibration.yaml을 건드리면 안 된다.

`generate.py`는 PNG를 300dpi로 박고 출고 전 `_assert_png_300dpi`로 검증하므로 **생성 시점에는 절대 깨지지 않는다.** 깨지는 곳은 항상 **그 이후의 전송 단계**다. 따라서 다음을 지켜 애초에 발생시키지 않는다.

- **기기 간 공유는 "파일"로 보낸다.** AirDrop·드라이브/디스크 업로드·파일 첨부는 PNG 메타를 보존한다. 카카오톡·슬랙·메일 **본문에 이미지로** 넣어 보내면 재인코딩되며 DPI가 72로 리셋된다 — 이 경로를 쓰지 말 것.
- **다른 기기로 넘길 때는 PDF를 마스터로 보낸다.** 같은 파이프라인이 만드는 `nametag-*.pdf`는 A4 물리 크기가 내부에 내장돼 전송에 안 깨진다. 받는 쪽에서 그 PDF로 로컬에서 PNG를 다시 뽑으면(아래) DPI가 로컬에서 재확정되어 전송 손상이 원천 차단된다. (인쇄 자체는 기존 불변식대로 300dpi PNG로 한다. PDF 직접 인쇄는 계속 금지.)
- **받은 파일은 출력 전 1초 점검**한다. 다른 기기에서 받았거나 코드 산출물명(`nametag-<타임스탬프>.png`)이 아닌 리네임 파일(`...-logo-...` 등)이면 특히 의심한다.

```bash
# 점검 — 픽셀 2479×3508인데 dpi가 300이 아니면 전송 중 깨진 것
sips -g pixelWidth -g pixelHeight -g dpiWidth -g dpiHeight <png>

# 깨졌을 때 복구 (최후수단) — 픽셀 그대로, DPI 태그만 300으로 (리샘플링 아님, 화질 손실 0)
sips -s dpiWidth 300 -s dpiHeight 300 <png> --out <png-300dpi>
```

복구는 어디까지나 손상된 파일을 받았을 때의 마지막 수단이다. 픽셀이 A4 비율(≈2479:3508)이면 메타 교체로 끝나고, 픽셀 자체가 다르면 재생성한다.
...
```text
탐사 A4 8칸 라벨지가 필요합니다.
구매 링크: https://link.coupang.com/a/eHTHl80SGq
